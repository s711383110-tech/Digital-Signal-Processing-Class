...
本題包含兩個 C 程式：
	1.	產生測試用的正弦與餘弦音訊波形。
	2.	對產生的音訊執行一階 RC 濾波，模擬指定的濾波運算式。

⸻

1. sine_wav_gen.c 產生測試訊號

這個程式會產生雙聲道的 wav 音訊檔，左聲道是正弦波，右聲道是餘弦波。
輸出格式為 16 位元的雙聲道 PCM，會自動建立 wav 檔頭。
...
其中
	...
	2.	建立 wav 檔頭
設定取樣率、取樣位元數和聲道數，並計算每秒資料位元率與封包大小。
	3.	產生樣本資料
利用 sin 和 cos 函數計算每個取樣點的振幅，並乘上一個適當的常數讓音量不會失真。
左聲道輸出 sin 波形，右聲道輸出 cos 波形。
	4.	輸出音訊檔
將兩個聲道的取樣點依序寫入檔案中形成雙聲道音訊。
...
這個程式會讀取前一個步驟產生的 wav 音訊，
並進行一階 RC 濾波，模擬簡單的低通濾波器。
...
其中
	•	in.wav 為輸入音訊檔
	•	out.wav 為輸出經過濾波後的音訊檔

	2.	讀取 wav 檔頭
確認格式正確，只支援 16 位元雙聲道的 PCM 音訊。
	3.	計算濾波係數
根據設定的截止頻率（例如 4000Hz）和取樣頻率計算濾波係數。
主要包含兩個參數 a1 和 b0，控制濾波強度。
	4.	濾波主程式
依序讀取每個取樣點，根據前一個輸出值和目前輸入值進行運算。
對左右聲道分別計算新的輸出樣本，然後寫入新的 wav 檔。
	5.	輸出結果
完成後會在終端機顯示處理完成訊息與實際使用的濾波參數。
...
sine_wav_gen.c
   產生 sin / cos 測試音訊
   ↓
輸出 sincos_fs8000_f3000_L1.0.wav
   ↓
RC_filtering.c
   執行一階 RC 濾波
   ↓
輸出 filtered_sincos_fs8000_f3000_L1.0.wav
...
這兩個程式完整實作了 Problem 7 的要求：
	•	sine_wav_gen.c 能依設定的頻率與取樣率產生雙聲道音訊。
	•	RC_filtering.c 會讀取音訊並進行一階 RC 濾波處理。
	•	輸出的 wav 音檔可直接播放，用以比較濾波前後的差異。
