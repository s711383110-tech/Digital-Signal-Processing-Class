...
- 當 \( f_s \) 越高 \( \Rightarrow T_s \) 越小 \( \Rightarrow \) 系統越接近連續時間 RC 模型。

- \( H(\omega) \) 的幅度曲線變平滑，且與連續系統的  
  \( H(j\omega) = \dfrac{1}{1 + j\omega RC} \) 越接近。

- 在低取樣率（例如 \( 4\,\text{kHz} \)）時，誤差較大；  
  而在高取樣率（例如 \( 16\,\text{kHz} \)）時，數位與模擬濾波器幾乎相同。
